...
const FileReview = () => {
      ...
      comments: [
        {
          ...
          message:
            "I've reviewed the structural details and they look comprehensive. The beam schedules are clear and coordination points are well marked.",
          reactions: { '👍': [2, 3], '🎉': [3] },
          edited: false,
          replies: [
            {
        ...
        },
        {
          ...
          message:
            "Please ensure HVAC duct routing in section A-A doesn't conflict with the structural members shown. 🚀",
          reactions: { '🚀': [2, 999], '👀': [3] },
          edited: true,
          replies: [],
        },
  ...
  const addComment = () => {
    ...
    updatedDocs[selectedDoc].comments.push(newCommentObj as any)
  ...
  const addReply = (commentId: number) => {
    ...
    const updatedDocs = [...documents]
    updatedDocs[selectedDoc].comments = updatedDocs[selectedDoc].comments.map((c: any) => {
      if (c.id === commentId) {
        return {
          ...c,
          replies: [
  ...
  const toggleReaction = (commentId: number, emoji: string, isReply = false, parentId: number | null = null) => {
    const updatedDocs = [...documents]
  ...
  const saveEdit = (commentId: number, isReply = false, parentId: number | null = null) => {
    const updatedDocs = [...documents]
  ...
  const deleteComment = (commentId: number, isReply = false, parentId: number | null = null) => {
    const updatedDocs = [...documents]
    ...
    if (isReply) {
      ...
      updatedDocs[selectedDoc].comments = updatedDocs[selectedDoc].comments.filter(
  ...
  const renderComment = (comment: any, isReply = false, parentId: number | null = null) => {
          ...
          <div className="flex items-center gap-2 mt-2">
            {Object.entries(comment.reactions || {}).map(([emoji, users]) => (
              <button
                key={emoji}
                onClick={() =>
                  toggleReaction(comment.id, emoji, isReply, parentId)
                }
  ...
  const renderLeftPanel = () => (
    <div className={`${showRightPanel ? 'border-r ' : ''}flex flex-col`}>
      {/* PDF Toolbar */}
      ...
      <ScrollArea className="flex-1 overflow-y-auto overflow-x-hidden">
        <div
          className={`p-4 sm:p-6 lg:p-8 flex items-center ${
            showRightPanel ? 'justify-center' : 'justify-start'
          } bg-muted/20`}
        >
          <div
            ...
            style={{
              ...
              width: '100%',
              maxWidth: showRightPanel ? '816px' : '100%',
              minHeight: 'min(1056px, 100vh - 200px)',
            }}
          >
            <iframe
          ...
          <TabsContent value="discussion" className="p-6 space-y-6 mt-0">
            {currentDoc.comments.length > 0 ? (
              <div className="space-y-4">
                {currentDoc.comments.map((comment: any) => renderComment(comment))}
              </div>
            ) : (
              <Card>
          ...
          <TabsContent value="reviewers" className="p-6 space-y-4 mt-0">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {
                    currentDoc.reviewers.filter(
                      (r: any) => r.status === 'approved',
                    ).length
                  }{' '}
                  of {currentDoc.reviewers.length} reviewers have approved
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                {currentDoc.reviewers.map((reviewer: any) => (
                  <div
                    key={reviewer.id}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Avatar>
                        <AvatarFallback
                          className={`${reviewer.color} text-white`}
                        >
                          {reviewer.initials}
                        </AvatarFallback>
                      </Avatar>
